...
Our work introduces a Multiscale Power-Law Initialization ($\alpha=2.0$) that transforms the ELM's random features into a robust physical basis. By utilizing heavy-tailed weight distributions, the EM-ELM generates a spectrum of neurons capable of capturing both global trends and sharp gradients simultaneously. This architectural choice effectively bypasses the spectral bias inherent in iterative methods. By formulating the training process as a Regularized Energy Minimization problem, we derive the output weights through a deterministic analytical solve. This approach provides a significant ``speed-accuracy'' advantage: on the 1-D Poisson benchmark our results demonstrate a measured $10.4\times$ speedup over a single-layer PINN and $358\times$ over a four-layer deep PINN, while maintaining competitive accuracy.
...
\begin{itemize}
    ...
    \item \textbf{Performance Benchmarking:} A measured $10.4\times$ wall-clock speedup over a single-layer PINN and $358\times$ over a four-layer deep PINN on the 1-D Poisson benchmark, demonstrating suitability for real-time edge computing and digital twins.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Analytical Certainty:} By solving the global energy minimum in one step, we avoid the non-convex optimization traps that caused the PINN to take 10.2\,s compared to the ELM's 0.975\,s.
\end{itemize}
...
To rigorously justify the empirical $358\times$ speedup observed in our results, we perform a formal computational complexity analysis comparing the proposed EM-ELM with the standard PINN framework.
...
\begin{itemize}
    \item \textbf{Accuracy:} The EM-ELM achieves a mean $L_2$ relative error of $\approx 0.107$, whereas the Gaussian-initialized ELM fails to reach a physically relevant solution with an error of $\approx 0.52$.
    \item \textbf{Precision vs.\ Speed:} While the Light PINN reaches a superior precision of $\approx 0.006$, it requires an average of 10.7\,s of training time.
    \item \textbf{Efficiency:} The EM-ELM provides a deterministic one-shot solve, yielding a $10.4\times$ increase in throughput over the Light PINN and a $358\times$ speedup over deep iterative architectures.
\end{itemize}
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Statistical stability and performance distribution.} The box-and-whisker plot illustrates the $L_2$ relative error distribution across 20 independent random seeds. The proposed Power-Law initialization ($\alpha=2.0$) significantly stabilizes the ELM framework, reducing mean error by approximately $80\%$ compared to standard Gaussian initialization. While the iterative PINN maintains superior precision, the EM-ELM provides a deterministic analytical solve that is roughly $10.4\times$ faster, establishing it as an ideal candidate for low-latency digital twin applications.}
    \label{fig:box_whisker_stats}
\end{figure}

\subsection{Extended Benchmarks}

To validate the generality of the Power-Law advantage beyond the single-frequency Poisson problem, we evaluate the EM-ELM on three additional benchmarks of increasing physical complexity. Table~\ref{tab:extended_benchmarks} summarises the results across all methods and problems, and Figures~\ref{fig:multifreq_poisson}--\ref{fig:burgers} present the corresponding solution fields.

\begin{table}[htbp]
\centering
\caption{\textbf{Extended benchmark results.} Mean $L_2$ relative error $\pm$ one standard deviation over 10 seeds. Bold indicates the best-performing method for each problem. The ``PL/G Ratio'' column reports the Gaussian-to-Power-Law error ratio ($>1$ means Power-Law wins).}
...
\multirow{3}{*}{\shortstack[l]{Multi-Freq\\Poisson 1D}}
  & ELM (Power Law) & 0.259 & 0.010 & 0.98 & \multirow{3}{*}{1.5$\times$} \\
  & ELM (Gaussian)  & 0.400 & 0.125 & 0.88 & \\
  & PINN (Light)    & \textbf{0.207} & 0.045 & 13.1 & \\
\midrule
\multirow{3}{*}{\shortstack[l]{Helmholtz\\($k=10$)}}
  & ELM (Power Law) & \textbf{0.183} & 0.084 & 1.13 & \multirow{3}{*}{3.7$\times$} \\
  & ELM (Gaussian)  & 0.668 & 0.160 & 0.98 & \\
  & PINN (Light)    & 0.339 & 0.029 & 15.2 & \\
\midrule
\multirow{4}{*}{\shortstack[l]{Burgers\\($\nu=0.01/\pi$)}}
  & ELM (Power Law) & 0.237 & 0.032 & 3.29 & \multirow{4}{*}{1.0$\times$} \\
  & ELM (Gaussian)  & 0.243 & 0.005 & 185.1 & \\
  & PINN (Light)    & \textbf{0.154} & 0.044 & 1863.9 & \\
  & PINN (Deep)     & 0.161 & 0.196 & 12279.6 & \\
\bottomrule
\addlinespace[1ex]
...
We consider a multi-frequency Poisson problem $-u''(x) = f(x)$ on $[-1,1]$ with manufactured solution
\begin{equation}
  u(x) = \sin(\pi x) + 0.3\sin(3\pi x) + 0.1\sin(7\pi x),
\end{equation}
which combines three spatial frequencies ($\pi$, $3\pi$, $7\pi$) with decreasing amplitudes. This target requires the random basis to simultaneously resolve a fundamental mode, a third harmonic, and a seventh harmonic.

As shown in Table~\ref{tab:extended_benchmarks}, the Power-Law ELM achieves a $1.5\times$ lower mean $L_2$ error than the Gaussian variant. Figure~\ref{fig:multifreq_poisson} illustrates that the Gaussian ELM systematically underresolves the higher harmonics, producing visible oscillation errors at the $3\pi$ and $7\pi$ components, while the Power-Law ELM tracks all three modes with higher fidelity.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{benchmark3_multifreq_poisson.png}
    \caption{\textbf{Multi-frequency Poisson 1D.} (A)~Reconstructed solution field: the Power-Law ELM captures all three frequency components ($\pi$, $3\pi$, $7\pi$), while the Gaussian ELM underresolves the higher harmonics. (B)~$L_2$ error distribution over 10 seeds. (C)~Pointwise absolute error.}
    \label{fig:multifreq_poisson}
\end{figure}

\subsubsection{Helmholtz Equation (High Wavenumber)}

We next solve the Helmholtz equation, a dispersive PDE central to acoustics and electromagnetics:
\begin{equation}
  -u''(x) - k^2 u(x) = f(x),\quad x \in [-1,1],\quad u(\pm 1)=0
\end{equation}
with wavenumber $k=10$ and manufactured solution $u(x) = \sin(\pi x)\,e^{-4x^2}$, a localised wave packet whose Gaussian envelope confines the oscillation to the interior of the domain. The $-k^2 u$ operator term amplifies any mismatch between the basis frequencies and the solution, making this benchmark particularly sensitive to the quality of the random feature space.

This benchmark produces the \emph{strongest} separation between initialisation strategies (Table~\ref{tab:extended_benchmarks}). The Power-Law ELM achieves $3.7\times$ lower mean error than the Gaussian ELM ($L_2 = 0.183$ vs.\ $0.668$), and even outperforms the PINN baseline ($0.339$). As shown in Figure~\ref{fig:helmholtz}, the Gaussian ELM fails to reconstruct the localised wave packet, while the Power-Law ELM captures both the oscillatory core and the rapid decay imposed by the envelope.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{benchmark4_helmholtz.png}
    \caption{\textbf{Helmholtz equation ($k=10$).} (A)~Solution reconstruction: the Gaussian ELM fails to capture the localised wave packet $u(x)=\sin(\pi x)\,e^{-4x^2}$, while the Power-Law ELM resolves both the oscillation and envelope. (B)~$L_2$ error distribution. (C)~Pointwise error showing uniform accuracy of the Power-Law basis.}
    \label{fig:helmholtz}
\end{figure}

\subsubsection{Viscous Burgers Equation}

As a final benchmark, we solve the viscous Burgers equation:
\begin{equation}
  u_t + u\, u_x = \nu\, u_{xx},\quad (x,t) \in [-1,1] \times [0,1],\quad u(x,0) = -\sin(\pi x)
\end{equation}
with viscosity $\nu = 0.01/\pi$. Unlike the previous elliptic benchmarks, Burgers is time-dependent and nonlinear, developing steep gradients near $x=0$ as the solution evolves. The reference solution is computed via a high-resolution method of lines solver ($n_x=1024$, RK45, tolerances $10^{-8}$/$10^{-10}$).

The ELM treats Burgers as a 2D regression problem over the $(x,t)$ domain. As shown in Table~\ref{tab:extended_benchmarks}, both ELM variants achieve comparable accuracy ($L_2 \approx 0.24$), with the Power-Law ELM slightly outperforming the Gaussian variant ($0.237$ vs.\ $0.243$) while being $56\times$ faster ($3.29$\,s vs.\ $185.1$\,s). The iterative PINN (Light) achieves the best accuracy ($0.154$) but at a cost of $1863.9$\,s---a $566\times$ slowdown relative to the Power-Law ELM. Figure~\ref{fig:burgers} presents the solution snapshot at $t=0.5$, comparing both ELM variants against the numerical reference.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{burgers_benchmark.png}
    \caption{\textbf{Viscous Burgers equation.} (A)~Reference solution at three time snapshots showing the development of steep gradients. (B)~Solution snapshot at $t=0.5$: comparison of ELM (Power-Law), ELM (Gaussian), and numerical reference. (C)~$L_2$ error distribution over 5 seeds.}
    \label{fig:burgers}
\end{figure}
...
The $358\times$ speedup (measured against a four-layer deep PINN) achieved by the EM-ELM over deep PINNs is not merely a numerical improvement; it represents a shift in deployment capability. While a 349-second training time (Deep PINN) is acceptable for offline analysis, it is prohibitive for real-time applications. The sub-second latency of the EM-ELM allows it to function as a ``Real-Time Digital Twin,'' capable of updating physical states in dynamic environments such as fluid flow monitoring or structural health sensing.
...
\begin{enumerate}
    ...
    \item The analytical projection achieves a $10.4\times$ speedup over a Light PINN and a $358\times$ speedup over deep iterative architectures, enabling sub-second inference suitable for real-time applications.
...
These results position the EM-ELM as a compelling candidate for real-time digital twin applications and edge computing scenarios where both speed and physical fidelity are critical. Extended benchmarks on a multi-frequency Poisson problem, a high-wavenumber Helmholtz equation ($k=10$), and the nonlinear viscous Burgers equation confirm that the Power-Law advantage grows with the multi-scale complexity of the target solution. Future work will extend the framework to coupled multi-physics systems and adaptive basis refinement strategies.
