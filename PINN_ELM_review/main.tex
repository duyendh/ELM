...
\usepackage[markup=default,draft]{changes}
...
\added[id=R1]{...the EM-ELM achieves a measured $10.7\times$ wall-clock speedup over a single-layer PINN and $389\times$ over a four-layer deep PINN on the 1-D Poisson benchmark. [...] A complementary classification experiment on MNIST and FashionMNIST confirms the generality of the approach.}
\bigskip
...
\begin{itemize}
    ...
    \item \textbf{Performance Benchmarking:} \replaced[id=R1]{A measured $10.7\times$ wall-clock speedup over a single-layer PINN and $389\times$ over a four-layer deep PINN on the 1-D Poisson benchmark, demonstrating suitability for real-time edge computing and digital twins.}{Realization of order-of-magnitude speedups ($>10\times$ for light models, $>150\times$ for deep models) suitable for real-time edge computing and digital twins.}
\end{itemize}
...
{\color{red}\textbf{NEED TO CHECK AND VERIFY}}\\[4pt]
\textbf{Review Note \#1 --- Weight Scaling:} The PDE notebook cells rescale all weights by a factor of $0.1$ (i.e.\ \texttt{weights * 0.1}). For $\sin(x \cdot w + b)$ with scalar input $x \in [-1,1]$, this places most neurons in the small-angle regime $\sin(\epsilon) \approx \epsilon$. It is unclear whether (a)~the power-law heavy tail produces enough large weights to escape this regime, or (b)~the reported accuracy would improve substantially with a larger scale. This should be tested empirically by running the benchmark at multiple scales (e.g.\ $0.1$, $1.0$, $10.0$) before finalising results. The classification cell (784-D input, sigmoid activation) is unaffected.}}
\bigskip
...
To rigorously justify the empirical $150\times$ speedup observed in our results, we perform a formal computational complexity analysis comparing the proposed EM-ELM with the standard PINN framework.
...
\begin{itemize}
    ...
    \item \textbf{Efficiency:} The EM-ELM provides a deterministic solve in 0.282\,s, yielding a $7.35\times$ increase in throughput over the Light PINN and a $150\times$ speedup over deep iterative architectures.
\end{itemize}
...
\begin{enumerate}
    ...
    \item The analytical projection achieves a $7.3\times$ speedup over a Light PINN and a $150\times$ speedup over deep iterative architectures, enabling sub-second inference suitable for real-time applications.
