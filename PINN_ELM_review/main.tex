...
\textbf{Review Note \#1 --- Weight Scaling:} The current notebook implementation rescales the Power-Law weights by a factor of $0.1$ (i.e.\ \texttt{weights * 0.1}). For $\sin(x \cdot w + b)$ with $x \in [-1,1]$, this collapses the argument to $\approx 0$, placing all neurons in the small-angle linear regime $\sin(\epsilon) \approx \epsilon$ and destroying the multiscale basis that the Power-Law distribution is designed to provide.}}
\bigskip
...
\noindent\fcolorbox{red}{red!10}{\parbox{\dimexpr\linewidth-2\fboxsep-2\fboxrule}{%
{\color{red}\textbf{NEED TO CHECK AND VERIFY}}\\[4pt]
\textbf{Review Note \#2 --- Optimal $\lambda$:} The choice of $\lambda = 10^{-6}$ and the claimed logarithmic sweep must be re-validated \emph{after} the weight-scaling issue (Review Note \#1) is resolved. With the current $0.1\times$ scaling the sweep produces a monotone curve favouring large $\lambda$; a corrected weight scale should yield the expected U-shaped bias--variance trade-off.}}
\bigskip
