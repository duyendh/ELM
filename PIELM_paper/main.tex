...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Power-law advantage across problem types.} Left: solutions; right: pointwise error. Rows 1--3 (Multi-Freq Poisson, Helmholtz $k{=}20$ at $h{=}20$ and $h{=}15$): log-uniform tracks the exact solution while others fail. Rows 4--5 (Poly $\times$ Oscillation, Gaussian + Oscillation): all methods struggle, but log-uniform degrades more gracefully. Row 6 ($\sin(\pi x)\cos(3\pi x)$ at $h{=}15$): log-uniform resolves the product structure.}
    \label{fig:diverse}
\end{figure}
